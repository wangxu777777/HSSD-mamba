...
class DenoiseDataSet(Dataset):
    ...
    def _load_img(self, img_name, as_gray=False):
        img = cv2.imread(img_name, 1)       # return ndarray [w,h,3]
        # img = cv2.imread(,img_name 0)     # return ndarray [w,h]
    ...
    def _load_img_from_np(self, img, as_gray=False, RGBflip=False):
        ...
        if len(img.shape) != 2:
            if as_gray:
                ...
                print('warning the img must be gray!')
                if RGBflip:
    ...
    def _get_patch(self, crop_size, data, rnd=True):
        ...
        if 'clean' in data:
            ...
            for key, value in data.items():
                # 查找第一个是 Tensor (图像) 并且不是 'file_name' 的键
                if isinstance(value, torch.Tensor) and 'file_name' not in key:
                    patch_key = key
                    break

            if patch_key is None:
                raise KeyError("在数据字典中没有找到用于裁剪的图像键 (例如 'noisy' 或 'real_noisy')。")

            # 使用找到的键 (patch_key) 来获取尺寸，而不是写死的 'real_noisy'
            max_x = data[patch_key].shape[2] - crop_size[0] # <--- 已修正
            max_y = data[patch_key].shape[1] - crop_size[1] # <--- 已修正

        assert max_x >= 0, f'hope max_x >= 0, but get {max_x}'
        assert max_y >= 0, f'hope max_y >= 0, but get {max_y}'
    ...
    def prep_save(self, img_idx:int, img_size:int, overlap:int, pad_needed:bool=True, clean:bool=False, syn_noisy:bool=False, real_noisy:bool=False):
        '''
        ...
        if h < img_size // 2 + 1 or w < img_size // 2 + 1:
            print('Skip image %d / %d\t%s' % (img_idx, self.__len__(), data['file_name']))
            return

        if pad_needed is True:
            h_pad_needed = stride - (h - img_size) % stride
            w_pad_needed = stride - (w - img_size) % stride
            data['real_noisy'] = F.pad(input=data['real_noisy'], pad=(w_pad_needed // 2, w_pad_needed - w_pad_needed // 2, h_pad_needed // 2, h_pad_needed - h_pad_needed // 2), mode='reflect')
            c, h, w = data['real_noisy'].shape

        for h_idx in range(((h - img_size) // stride) + 1):
            for w_idx in range(((w - img_size) // stride) + 1):
                hl, hr = h_idx * stride, h_idx * stride + img_size
                wl, wr = w_idx * stride, w_idx * stride + img_size
                if hr > h or wr > w:
                    print(f'[{hl},{hr}], [{wl},{wr}], but shape is [{h},{w}], h_idx: {h_idx}/ range {((h - img_size) // stride)}, w_idx: {w_idx}/ range {(((w - img_size + 1) // stride) )}')
                ...
                if real_noisy: cv2.imwrite(os.path.join(real_noisy_dir, '%d_%s_%d_%d.png' % (img_idx, data['file_name'], h_idx, w_idx)), tensor2np(data['real_noisy'][:, hl:hr, wl:wr]))

        print('Cropped image %d / %d\t%s' % (img_idx, self.__len__(), data['file_name']))
