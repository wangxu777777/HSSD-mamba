...
#         return unshuffled.view(c,f,f,w//f+2*pad,h//f+2*pad).permute(0,1,3,2,4).reshape(c, w+2*f*pad, h+2*f*pad)  # 变成那种一张大图上好多个小子图的情况了
...
def create_scripts_folder(save_path, source_path):
    # if not os.path.exists(save_path):
    #     os.mkdir(save_path)
    # print('Experiment logging folder dir : {}'.format(save_path))
    time_stamp = datetime.datetime.now().strftime('%d_%H_%M')
    target_folder_path = os.path.join(save_path, f'scripts_{time_stamp}')
    ...
    if os.path.exists(source_path):
        ...
        for root, dirs, files in os.walk(source_path):
            ...
            if 'check' in root or 'dataset' in root or 'md' in root or 'output' in root or 'dnd' in root or 'result' in root :
                continue

            relative_path = root[len(source_path):]  # 跳过source后面的 /
            # print(relative_path)
            # print(f'check {relative_path}')

            """不再额外备份保存数据集文件夹"""
            # if 'data' in relative_path and 'datasets' not in relative_path:
            #     continue
            #
            # if 'checkpoint' in relative_path:
            #     continue

            target_path = os.path.join(target_folder_path, relative_path)
